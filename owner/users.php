...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'reset_password':
            // Owner can reset admin, support, supplier — but not other owners or self.
            if ($uid > 0 && $uid !== $userId) {
                // Fetch target user to determine their role and email.
                $tgtRow = $pdo->prepare(
        ...
        case 'generate_invitation':
            ...
            if ($invRole === 'admin') {
                ...
                if (empty($checkedIds)) {
                    ...
                    header('Location: /login/owner/users.php');
                    exit;
                }
                $targetOrgId = $checkedIds[0];
                $extraOrgIds = array_slice($checkedIds, 1);
            } else {
                $targetOrgId = (int) ($_POST['org_id'] ?? ($accessibleOrgIds[0] ?? 0));
...
$uStmt->execute([$usersPerPage, $usersOffset]);
...
// ── Invitations data ──────────────────────────────────────────
$orgs        = $accessibleOrgs; // all orgs for invitation form dropdown
$invitations = [];
    ...
    <div class="page-content">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="user-actions">
                                    ...
                                    <!-- Row 2: role change -->
                                    <form method="POST" action="/login/owner/users.php" class="user-actions-row">
                                        <input type="hidden" name="csrf_token"
                                               value="<?= htmlspecialchars(csrfToken(), ENT_QUOTES, 'UTF-8') ?>">
                                        <input type="hidden" name="user_id" value="<?= (int) $u['id'] ?>">
                                    ...
                                    <?php if ($primaryRole !== 'owner'): ?>
                                    <!-- Row 3: reset password (not available for other owners) -->
                                    <div class="user-actions-row">
                                        <form method="POST" action="/login/owner/users.php"
        ...
        <section class="panel-section" style="margin-top:36px;" id="invitations">
            ...
            <div style="margin-top:24px;">
                ...
                <form method="POST" action="/login/owner/users.php#invitations" class="inv-gen-form">
                    <?= csrfField() ?>
                    <input type="hidden" name="action" value="generate_invitation">
                    <div class="form-row">
                        <!-- Org selector — shown when non-admin role selected -->
                        ...
                        <!-- Multi-org checkboxes — shown when admin role is selected -->
        ...
        setInterval(function () {
            ...
            if (idle >= TIMEOUT_MS) {
                ...
                if (window.confirm('Su sesión cerrará pronto por inactividad. ¿Desea continuar?')) {
                    resetTimer();
                    fetch('/login/owner/users.php', { method: 'HEAD', credentials: 'same-origin' });
                }
            }
        }, 10000);
    })();
